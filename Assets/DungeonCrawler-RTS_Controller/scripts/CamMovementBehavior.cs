...
public class CamMovementBehavior : MonoBehaviour
{
    ...
    private void DetermineKeyPressEvent(Transform selected, Transform active)
    {
        ...
        if (Input.GetAxis("Mouse ScrollWheel") > 0)
        {
            ...
            {
                if ((selected.position - camera.transform.position).magnitude > MinZoomDistance)
                {
            ...
            {
                if ((TargetPosition - camera.transform.position).magnitude > MinZoomDistance)
                {
                    //zoom_distance += (ZoomSpeed * Time.deltaTime * 60);
                    camera.transform.position += camera.transform.forward * (ZoomSpeed * Time.deltaTime * 60);
                    TargetPositionCalculated = false;
                }
            }
        }
        ...
        else if (Input.GetAxis("Mouse ScrollWheel") < 0)
        {
            ...
            {
                if ((selected.position - camera.transform.position).magnitude < MaxZoomDistance)
                {
                    zoom_distance -= (ZoomSpeed * Time.deltaTime * 60);
                    TargetPositionCalculated = false;
                }
            ...
            {
                if ((TargetPosition - camera.transform.position).magnitude < MaxZoomDistance)
                {
                    //zoom_distance -= (ZoomSpeed * Time.deltaTime * 60);
                    camera.transform.position += camera.transform.forward * -(ZoomSpeed * Time.deltaTime * 60);
                    TargetPositionCalculated = false;
    ...
    private void DoDefaultCameraBehavior()
    {
        ...
        {
            Ray cam_ray = new Ray(camera.transform.position, camera.transform.forward);
