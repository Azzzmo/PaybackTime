...
public class ControlObjHandler : MonoBehaviour 
{
    ...
    void OnGUI()
    {
        ...
        if (selectedIndicator != null && selectedIndicator.gameObject.activeSelf)
        {
            ...
            {
                Vector3 LLparentPos = Camera.mainCamera.WorldToScreenPoint(transform.position + (transform.localScale.magnitude / 2 * -Camera.mainCamera.transform.right - new Vector3(0, transform.localScale.y + transform.localScale.y / 2, 0)));
                Vector3 ULparentPos = Camera.mainCamera.WorldToScreenPoint(transform.position + (transform.localScale.magnitude / 2 * -Camera.mainCamera.transform.right + new Vector3(0, transform.localScale.y + transform.localScale.y / 2, 0)));
                Vector3 LRparentPos = Camera.mainCamera.WorldToScreenPoint(transform.position + (transform.localScale.magnitude / 2 * Camera.mainCamera.transform.right - new Vector3(0, transform.localScale.y + transform.localScale.y / 2, 0)));
                Vector3 URparentPos = Camera.mainCamera.WorldToScreenPoint(transform.position + (transform.localScale.magnitude / 2 * Camera.mainCamera.transform.right + new Vector3(0, transform.localScale.y + transform.localScale.y / 2, 0)));
    ...
    public static List<Transform> DetermineInMultiSelect(Vector2 init, Vector2 end)
    {
        ...
        {
            Vector2 trans2screen = Camera.mainCamera.WorldToScreenPoint(trans.position);
